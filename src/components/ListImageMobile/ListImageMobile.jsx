...
import { getCarouselImages, deleteImage, editImage, getCarouselImagesMobile, deleteImageMobile, editImageMobile } from "@/firebase/firebaseServices";
import { ToastContainer, toast } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";

export default function CarouselManagerMobile() {
  ...
  const fetchImages = async () => {
    setLoading(true);
    try {
      const data = await getCarouselImagesMobile();
  ...
  const handleDelete = async (docId, imageUrl) => {
    ...
    setLoading(true);
    try {
      await deleteImageMobile(docId, imageUrl);
  ...
  const handleEdit = async (docId, oldImageUrl, e) => {
    ...
    setLoading(true);
    try {
      await editImageMobile(docId, oldImageUrl, file);
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 bg-white shadow-md rounded-lg mt-20">
      <h2 className="text-lg font-bold mb-3 text-center">Gestionar Carrusel para Mobile</h2>
